...
public class LCPApp implements SceneGraphNode, Interactor {
  ...
  /**
   * boolean to signal that the system was stepped and that a frame should be recorded if recording
   * is enabled
   */
  ...
  public LCPApp() {
    ...
    ev =
        new EasyViewer(
            "2D Rigid Body Collision Processing",
            this,
            new Dimension(540, 480),
  ...
  public void display(GLAutoDrawable drawable) {
    ...
    if (run.getValue() || stepped) {
      ...
      if (record.getValue()) {
        ...
        EasyViewer.beginOverlay(drawable);
        String recordInfo =
            "RECORDED: " + numRecordedFrames + " frames to " + videoFileName.getText();
        gl.glDisable(GL2.GL_LIGHTING);
        gl.glColor4f(1, 0, 0, 1);
        EasyViewer.printTextLines(
            drawable,
            recordInfo,
            10,
  ...
  /**
   * Resets the rigid body system, and factory if it is currently being used. When the factory is
   * reset, all non pinned rigid bodies are removed from the system.
   */
  ...
  /**
   * Clears the system, and ensures that any display lists that were created to draw the various
   * rigid bodies are cleaned up on the next display call.
   */
          ...
          public void mouseDragged(MouseEvent e) {
            ...
            if (picked == null) {
              ...
              if ((e.getModifiers() & InputEvent.BUTTON3_MASK) != 0) {
                scale.setValue(
                    scale.getValue() * Math.pow(1.002, e.getPoint().y - prevMousePoint.y));
              }
            }
