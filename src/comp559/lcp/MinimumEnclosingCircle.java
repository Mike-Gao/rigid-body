...
/**
 * Algorithm for computing Minimum Enclosing Circle using Welzl's algorithm based on Sunshine's
 * implementation (www.sunshine2k.de)
 *
 * @author kry
 */
public class MinimumEnclosingCircle {
  ...
  public MinimumEnclosingCircle(Collection<Point2d> points) {
    try {
      ...
      System.err.println(
          "Welzl algo blowing the stack away ("
              + points.size()
              + " points), so building conservative AABB instead.");
  ...
  /**
   * Calculates the minimal enclosing circle recursively Call initially with boundary array empty
   * and b = 0. NOTE: stack overflow will occur for a large number of points!
   *
