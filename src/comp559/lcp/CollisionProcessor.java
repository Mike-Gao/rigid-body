...
/**
 * Class for detecting and resolving collisions. Currently this class uses penalty forces between
 * rigid bodies.
 *
 * @author kry
 */
public class CollisionProcessor {
  ...
  /**
   * The visitID is used to tag boundary volumes that are visited in a given time step. Marking
   * boundary volume nodes as visited during a time step allows for a visualization of those used,
   * but it can also be used to more efficiently update the centeres of bounding volumes (i.e., call
   * a BVNode's updatecW method at most once on any given timestep)
   */
  int visitID = 0;
  ...
  private DoubleParameter contactSpringStiffness =
      new DoubleParameter("penalty contact stiffness", 1e3, 1, 1e5);
  /** Viscous damping coefficient for the contact penalty spring */
  private DoubleParameter contactSpringDamping =
      new DoubleParameter("penalty contact damping", 10, 1, 1e4);
  /**
   * Threshold for the relative velocity in the normal direction, for determining if spring force
   * will be applied.
   */
  private DoubleParameter separationVelocityThreshold =
      new DoubleParameter(
          "penalty separation velocity threshold (controls bounce)", 1e-9, 1e-9, 1e3);
  /** Enables the contact penalty spring */
  private BooleanParameter enableContactSpring =
      new BooleanParameter("enable penalty contact spring", true);
  /** Enables damping of the contact penalty spring */
  private BooleanParameter enableContactDamping =
      new BooleanParameter("enable penalty contact damping", true);
  ...
  public void processCollisions(double dt) {
    ...
    if (contacts.size() > 0 && doLCP.getValue()) {
      ...
      // TODO: Objective 3 - Compute velocity update with iterative solve of contact constraint
      // matrix.
      ...
      for (int iter = 0; iter < iterations.getValue(); iter++) {
        ...
        for (Contact cnct : contacts) {
          ...
          for (int i = 0; i < 3; i++) {
            lambdaNormal -=
                cnct.jacobianRowOne[i + 0]
                    / dMat[2 * cnct.index]
                    * deltaV[3 * cnct.body1.index + i];
            lambdaNormal -=
                cnct.jacobianRowOne[i + 3]
                    / dMat[2 * cnct.index]
                    * deltaV[3 * cnct.body2.index + i];
          }
          ...
          for (int i = 0; i < 3; i++) {
            lambdaFriction -=
                deltaV[3 * cnct.body1.index + i]
                    * cnct.jacobianRowTwo[i]
                    / dMat[2 * cnct.index + 1];
            lambdaFriction -=
                deltaV[3 * cnct.body2.index + i]
                    * cnct.jacobianRowTwo[i + 3]
                    / dMat[2 * cnct.index + 1];
          }
  ...
  /**
   * Checks for collisions between bodies. Note that you can optionaly implement some broad phase
   * test such as spatial hashing to reduce the n squared body-body tests. Currently this does the
   * naive n squared collision check.
   */
  ...
  /**
   * Checks for collision between boundary blocks on two rigid bodies. TODO: Objective 2 - This
   * needs to be improved as the n-squared block test is too slow!
   *
   * @param body1
   * @param body2
   */
  ...
  /**
   * Resets the state of the collision processor by clearing all currently identified contacts, and
   * reseting the visitID for tracking the bounding volumes used
   */
  public void reset() {
    contacts.clear();
    Contact.nextContactIndex = 0;
  ...
  /**
   * Processes a collision between two bodies for two given blocks that are colliding. Currently
   * this implements a penalty force
   *
   * @param body1
   * @param b1
   * @param body2
