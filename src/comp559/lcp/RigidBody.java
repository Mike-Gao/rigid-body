...
public class RigidBody {
  ...
  public static double kineticEnergyThreshold = 1e-6;
  /** Map to keep track of display list IDs for drawing our rigid bodies efficiently */
  private static HashMap<ArrayList<Block>, Integer> mapBlocksToDisplayList =
      new HashMap<ArrayList<Block>, Integer>();
  ...
  /**
   * Deletes all display lists. This is called when clearing all rigid bodies from the simulation,
   * or when display lists need to be updated due to changing transparency of the blocks.
   *
   * @param gl
   */
  public static void clearDisplayLists(GL2 gl) {
   ...
   * (which are then set to zero), and then updating position and angle. The internal rigid
   * transforms are also updated.
   *
   * @param dt step size
   */
  ...
  /**
   * Computes the velocity of the provided point provided in world coordinates due to motion of this
   * body.
   *
   * @param contactPointW
  ...
  /**
   * Draws the center of mass position with a circle. The circle will be drawn differently if the
   * block is at rest (i.e., close to zero kinetic energy)
   *
   * @param drawable
   */
