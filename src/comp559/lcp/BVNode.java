...
/**
 * Bounding volume node used to build bounding volume trees. The tree is constructed such that every
 * interior node has two children. Either there are two children, or this is a leaf node and the
 * leafBlock member stores the block at this leaf. (Note that it is actually a bounding area, but it
 * is probably easier to stick with the 3D terminology)
 *
 * @author kry
 */
public class BVNode {
  ...
  /**
   * Create a bounding volume node from a list of blocks. An axis aligned bounding box is computed
   * for the blocks, and the blocks are split in the axis aligned direction in which they are most
   * spread out.
   *
   * @param blocks
   * @param body
  ...
  /**
   * Draws the bounding volume spheres at the boundary where the sphere was visited by its children
   * were not.
   *
   * @param drawable
   * @param visit
  ...
  public void displayVisitBoundary(GLAutoDrawable drawable, int visit) {
    if (isLeaf()) {
      boundingDisc.display(drawable);
    } else if (child1.visitID
        != visit) { // both children are visited, or not, never one or the other
      boundingDisc.display(drawable);
    } else {
